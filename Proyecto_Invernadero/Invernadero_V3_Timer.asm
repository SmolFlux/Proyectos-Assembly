...
__CONFIG  _XT_OSC & _WDT_OFF & _PWRTE_ON & _BODEN_OFF & _LVP_OFF ; Configuración de fusibles
     CBLOCK 0x20	
		...
		Auxiliar
		RESH				;Inversión de humedad - alto
		RESL				;Inversión de humedad - bajo 
...
;-----------------------------------;Se configura el ADCON0
	...
	movwf	TMR0			;Comienza la cuenta del TIMER0
...
INTTM0
	btfss	INTCON,T0IF		;Si la bandera está en alto, hubo una interrupcion
	goto 	FinInterrupcion	;Si no fue una interrupción por bandera, entonces no actualizamos la temp.
	call	Reloj			
	call	CONVERSION		
FinInterrupcion				;Termina la interrupcion 
	bcf	INTCON,T0IF			
	retfie					
